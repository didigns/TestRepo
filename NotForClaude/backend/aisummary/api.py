...
# ---- inference queue -------------------------------------------------
# Low-end machines can't run several LLM answers at once. Every heavy query
# (chat, streaming, quick-ask, bots) passes through this gate so only ONE runs
# at a time; the rest queue and start when the current one finishes.
import threading as _threading

_QGATE = _threading.Semaphore(1)     # capacity 1 => strictly serial
_QWAIT = {"n": 0}
_QWLOCK = _threading.Lock()


def _queue_waiting() -> int:
    with _QWLOCK:
        return _QWAIT["n"]


class _QueueGate:
    """Context manager: acquire the single inference slot, queueing if busy."""
    def __enter__(self):
        if not _QGATE.acquire(blocking=False):
            with _QWLOCK:
                _QWAIT["n"] += 1
            try:
                _QGATE.acquire()          # block until the slot frees
            finally:
                with _QWLOCK:
                    _QWAIT["n"] -= 1
        return self
...
def _bot_answer(question: str) -> str:
    """Answer a question coming from a chat-app bot, using the live RAG engine.
    Optionally appends the source filenames so replies stay grounded. Goes
    through the inference queue so bot questions don't pile onto chat queries."""
    with _QueueGate():
        ans = _state["engine"].query(question)
    text = ans.text or "근거 문서에서 답을 찾지 못했습니다."
    if getattr(_state["settings"], "bot_reply_citations", True):
        files = []
        for c in getattr(ans, "citations", []) or []:
            fn = getattr(c, "filename", None) or (c.get("filename") if isinstance(c, dict) else None)
...
def _sync_bots():
    """Start/stop Discord & Slack bots to match settings (auto-run on startup)."""
    try:
        from .bots import MANAGER
        MANAGER.sync(_state["settings"], _bot_answer)
    except Exception:
        pass
...
def _boot():
    ...
    _sync_bots()   # auto-run enabled bots; re-sync on every settings change
...
class SettingsReq(BaseModel):
    ...
    bot_reply_citations: Optional[bool] = None
...
def update_settings(req: SettingsReq, _=Depends(auth)):
    ...
    if req.discord_bot_token is not None:
        s.discord_bot_token = req.discord_bot_token.strip()
    if req.discord_enabled is not None:
        s.discord_enabled = req.discord_enabled
    if req.slack_bot_token is not None:
        s.slack_bot_token = req.slack_bot_token.strip()
    if req.slack_app_token is not None:
        s.slack_app_token = req.slack_app_token.strip()
    if req.slack_enabled is not None:
        s.slack_enabled = req.slack_enabled
    if req.bot_reply_citations is not None:
        s.bot_reply_citations = req.bot_reply_citations
    save_settings(s)
    _boot()  # rebuild with new tier/models (also re-syncs bots)
...
@app.get("/bots/status")
def bots_status(_=Depends(auth)):
    """Connection status of the Discord / Slack bots."""
    from .bots import MANAGER
    return MANAGER.status()


@app.post("/bots/restart")
def bots_restart(_=Depends(auth)):
    """Stop and re-sync the bots with current settings."""
    from .bots import MANAGER
    MANAGER.stop_all()
    _sync_bots()
...
def query_stream(req: QueryReq, _=Depends(auth)):
    ...
    def gen():
        # take the single inference slot; if busy, tell the client it's queued
        # and block until the current answer finishes.
        if not _QGATE.acquire(blocking=False):
            with _QWLOCK:
                _QWAIT["n"] += 1
            try:
